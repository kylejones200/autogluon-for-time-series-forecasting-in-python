...
**AutoGluon** is auto ML Python library that streamlines feature
engineering, model selection, hyperparameter tuning, and evaluation.
AutoGluon has a `TimeSeriesPredictor`
...
dataset, initializing the `TimeSeriesPredictor` , training models automatically, and generating
forecasts.
...
- **item_id_column (optional)**: Identifies different time series in a
  dataset (for multivariate forecasting).
